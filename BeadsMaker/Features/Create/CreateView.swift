...
struct CreateView: View {
    ...
    private var editorCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Divider()
                .background(BeadsMakerTheme.outline)
    ...
    private var lockControlButton: some View {
        Button {
            ...
            .foregroundStyle(createStore.isCanvasLocked ? Color.white : BeadsMakerTheme.ink)
            .frame(height: 44)
            .padding(.horizontal, 12)
            .background(createStore.isCanvasLocked ? BeadsMakerTheme.ink : Color.white)
            .clipShape(RoundedRectangle(cornerRadius: BeadsMakerTheme.Radius.button, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: BeadsMakerTheme.Radius.button, style: .continuous)
                    .stroke(createStore.isCanvasLocked ? BeadsMakerTheme.ink : BeadsMakerTheme.outline, lineWidth: 1)
            )
            .contentShape(RoundedRectangle(cornerRadius: BeadsMakerTheme.Radius.button, style: .continuous))
        }
        .buttonStyle(.plain)
    ...
    private func toolSelectionButton(_ tool: EditorTool) -> some View {
        Button {
            ...
            .foregroundStyle(isToolButtonSelected(tool) ? Color.white : BeadsMakerTheme.ink)
            .frame(height: 44)
            .padding(.horizontal, 12)
            .background(isToolButtonSelected(tool) ? BeadsMakerTheme.ink : Color.white)
            .clipShape(RoundedRectangle(cornerRadius: BeadsMakerTheme.Radius.button, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: BeadsMakerTheme.Radius.button, style: .continuous)
                    .stroke(isToolButtonSelected(tool) ? BeadsMakerTheme.ink : BeadsMakerTheme.outline, lineWidth: 1)
            )
            .contentShape(RoundedRectangle(cornerRadius: BeadsMakerTheme.Radius.button, style: .continuous))
        }
        .buttonStyle(.plain)
    ...
    private func fitCanvasToView() {
        ...
        let cellSize = baseCellSize(
            width: createStore.currentPattern.width,
            height: createStore.currentPattern.height
        )
        let boardW = CGFloat(createStore.currentPattern.width) * cellSize
        let boardH = CGFloat(createStore.currentPattern.height) * cellSize
        let fitW = canvasViewportSize.width / boardW
        let fitH = canvasViewportSize.height / boardH
        canvasScale = (min(fitW, fitH) * 0.85).clamped(to: minScale...maxScale)
    }
    ...
    private struct BeadSwatch: View {
        ...
        var body: some View {
            ZStack {
                ...
                if isSelected {
                    Circle()
                        .stroke(BeadsMakerTheme.ink, lineWidth: 3)
