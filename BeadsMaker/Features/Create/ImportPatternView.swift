...
struct ImportPatternView: View {
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 10) {
                Image(systemName: "qrcode.viewfinder")
                    ...
                    .foregroundStyle(BeadsMakerTheme.ink.opacity(0.25))

                Text(L10n.tr("Scan a BeadsMaker QR code"))
                    .font(.headline)
                Text(L10n.tr("Point your camera at a QR code shared from another BeadsMaker user."))
                    .font(.subheadline)
    ...
    private var permissionDeniedView: some View {
        VStack(spacing: 20) {
            ...
            Image(systemName: "camera.fill")
                .font(.system(size: 48))
                .foregroundStyle(BeadsMakerTheme.ink.opacity(0.2))
    ...
    private func handleDetection(_ code: String) {
        guard let payload = PatternQRCode.decode(code) else {
            withAnimation { parseError = L10n.tr("Could not parse this QR code. Make sure it is a valid BeadsMaker pattern.") }
            return
        }
        parseError = nil
