...
final class LocalPatternStorage: PatternStorage {
    ...
    init(root: URL? = nil) {
        self.root = root ??
            ...
            .appendingPathComponent("BeadsMaker", isDirectory: true)
    }
...
final class iCloudPatternStorage: PatternStorage {
    ...
    init?(containerID: String = "iCloud.com.kevinzhang.beadsmaker") {
        guard let container = FileManager.default.url(forUbiquityContainerIdentifier: containerID) else {
            return nil
        }
        self.root = container.appendingPathComponent("BeadsMaker", isDirectory: true)
