...
> 模拟器测试（2026-04-25）：Xcode 26.1 Beta（17B55）存在 `xcodebuild` crash（exit 139）+ 模拟器不出现在 Device Picker 的已知 bug，导致 StoreKit Scheme 配置无法通过模拟器生效。StoreKit 相关测试需在真机（Xcode Run → Kevin's iPhone）完成。

**UI 层（模拟器 iPhone 16 验证）：**
- ✅ Profile 页"升级 Pro"CTA → PaywallView 弹出，"Not Now"正确关闭
- ✅ Paywall 展示：王冠图标 / "Become a Pro Creator" / 三项功能（发布图纸、无限草稿、iCloud 同步）/ ¥6 买断价格卡 / 购买 + 恢复按钮
- ✅ 草稿上限 Alert + "升级 Pro" → PaywallView 路径代码验证（手动触发需 20 草稿，跳过）
- ✅ `ProStatusManagerTests` 单元测试 6 项通过（升级 Pro、linkAppleAccount、appleUserID 编解码、PurchaseResult 枚举）

**真机流程（Debug bypass，Kevin's iPhone）：**
- ✅ 购买按钮点击 → ConfirmNameView 弹出（debug bypass：StoreKit + Apple Sign In 均绕过）
- ✅ ConfirmNameView 展示正常，可输入名称
- ✅ 杀进程重启 → Keychain Pro 状态保持，Profile 直接显示 Pro（修复了启动时 `sessionStore` 未同步 `ProStatusManager.isPro` 的 bug）
- ⬜ StoreKit 真实购买弹窗（需付费 Apple Developer 账号，Phase 5 上线准备时补测）
- ⬜ Apple Sign In 真实流程（同上）
- ⬜ Supabase users 表写入（依赖真实 Apple Sign In，同上）
- ⬜ 换设备恢复购买（需第二台真机 + 付费账号）

**测试中发现并修复的 bug：**
- 🐛 `ASAuthorizationController` 未持有为实例变量，delegate 回调前被 ARC 释放 → 修复
- 🐛 `presentationAnchor` 使用废弃 `.windows` API，iOS 16+ 返回空 → 改用 `.keyWindow`
- 🐛 App 启动时 `sessionStore.upgradeToPro()` 未在 Keychain 已有 Pro 时被调用 → 修复 `PixelBeadsApp.swift` 启动逻辑

**回归：**
- ✅ Phase 0 + Phase 1 功能在当前 build 正常（Explore 加载、草稿创建、Library 展示）

**阶段完成标记：⬜ → ✅ → 🧪**
**完成日期：2026-04-25（StoreKit/Apple Sign In 真实流程待付费账号后补测）**
