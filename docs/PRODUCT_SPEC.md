...
**真机 StoreKit 流程（需用 Kevin's iPhone + Xcode Scheme 运行）：**
- ⬜ 点"发布图纸"（未购买）→ PreviewView Paywall 弹出
- ⬜ 点购买 → StoreKit 本地测试弹窗 → 成功 → 触发 Apple Sign In
- ⬜ Apple Sign In 完成 → ConfirmNameView 展示，预填显示名称
- ⬜ 修改名称后保存 → Supabase users 表写入
- ⬜ 杀进程重启 → Keychain Pro 状态保持，无需重新购买
- ⬜ 换设备"恢复购买"→ Pro 状态恢复（跳过：需第二台真机）
- ⬜ 购买后 Pro 用户草稿数量无上限
...
**阶段完成标记：⬜ → ✅ → 🧪**
**完成日期：___________ （StoreKit 真机测试通过后更新）**
