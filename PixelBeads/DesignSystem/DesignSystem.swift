...
struct PBCardModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            ...
            .shadow(color: PixelBeadsTheme.shadow, radius: 16, y: 8)
    }
}
...
extension View {
    ...
    func pbScreen() -> some View {
        frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .top)
            .background(PixelBeadsTheme.surface.ignoresSafeArea())
...
struct PrimaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .padding(.vertical, 14)
            .background(PixelBeadsTheme.ink.opacity(configuration.isPressed ? 0.86 : 1))
